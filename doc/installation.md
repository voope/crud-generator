...
To get started, you should add the `voope/crud-generator` Composer dependency to your project:
```
composer require voope/crud-generator --dev
```
Once the package is installed, you should register the `Voope\CrudGenerator\CrudGeneratorServiceProvider` service provider. Normally, Laravel 5.5+ will register the service provider automatically.

After that, publish its assets using the `vendor:publish` Artisan command:
```
php artisan vendor:publish --provider="Voope\CrudGenerator\CrudGeneratorServiceProvider"
```
...
public function register()
{
    if ($this->app->environment() == 'local') {
        $this->app->register('Voope\CrudGenerator\CrudGeneratorServiceProvider');
    }
}
```
